...
function ensureCards(cb){if(window.CARDS&&window.CARDS.length){CARDS=window.CARDS;return cb();}if(!_cardsP){_cardsP=new Promise(function(res){var s=document.createElement("script");s.src="cards.js?v=w2";s.onload=function(){CARDS=window.CARDS||[];if(typeof applyPacks==="function")applyPacks();if(typeof applyJelajah==="function")applyJelajah();res();};s.onerror=function(){res();};document.head.appendChild(s);});}_cardsP.then(cb);}
