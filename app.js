...
function ensureCards(cb){if(window.CARDS&&window.CARDS.length){CARDS=window.CARDS;return cb();}if(!_cardsP){_cardsP=new Promise(function(res){var s=document.createElement("script");s.src="cards.js?v=w6";s.onload=function(){CARDS=window.CARDS||[];if(typeof applyPacks==="function")applyPacks();if(typeof applyJelajah==="function")applyJelajah();res();};s.onerror=function(){res();};document.head.appendChild(s);});}_cardsP.then(cb);}
...
/* ===== メインタブ ルーター ===== */
const SECTIONS=[
  {id:"learn",label:"学ぶ",icon:"i-book",tabs:[["learn","ことば"],["practice","練習"]]},
...
document.querySelectorAll(".subtabs[data-sub]").forEach(st=>{
  st.addEventListener("click",e=>{
    const g=e.target.closest("[data-grouptoggle]");
    if(g){const box=g.closest(".view"),row=box&&box.querySelector(".subtabs.sub2");
      if(row){const open=row.style.display!=="none";row.style.display=open?"none":"flex";g.classList.toggle("open",!open);}return;}
    const b=e.target.closest("[data-pane]");if(!b)return;const box=b.closest(".view");
    box.querySelectorAll(".subtabs button").forEach(x=>x.classList.toggle("active",x===b));
...
function packThreshold(id){var dots=_archBuild(),mx=-1,ci=-1;
  ...
  dots.forEach(function(d){if(d.ci===ci&&d.rank>mx)mx=d.rank;});return Math.ceil((mx+1)*ARCH_BASE/dots.length);}
...
function renderArch(el){const dots=_archBuild(),total=dots.length;
  ...
  const lit=Math.min(total,Math.round(known*total/ARCH_BASE)),pct=Math.round(lit/total*100);
  ...
  else{let best=inc[0],rem=1e9;inc.forEach(ci=>{const r=Math.ceil((maxRank[ci]+1)*ARCH_BASE/total)-known;if(r<rem){rem=r;best=ci;}});
...
function renderArchHome(){var el=$("homeArch");if(!el)return;var dots=_archBuild(),total=dots.length;
  ...
  else{var best=inc[0],rem=1e9;inc.forEach(function(ci){var r=Math.ceil((maxRank[ci]+1)*ARCH_BASE/dots.length)-known;if(r<rem){rem=r;best=ci;}});
...
renderProverb();
function openTarget(g){const[v,pane]=g.split(":");showView(v);if(pane){const box=document.querySelector('.view[data-view="'+v+'"]');if(box){box.querySelectorAll('.subtabs[data-sub] button').forEach(x=>x.classList.toggle("active",x.dataset.pane===pane));const tb=box.querySelector('.subtabs button[data-pane="'+pane+'"]'),row=tb&&tb.closest(".subtabs.sub2");if(row){row.style.display="flex";const gt=box.querySelector("[data-grouptoggle]");if(gt)gt.classList.add("open");}box.querySelectorAll(".pane").forEach(p=>p.classList.toggle("active",p.id===pane));initPane(pane);}}}
...
function _nextIsland(){try{var dots=_archBuild();var known=Object.values(LS("dks_status",{})).filter(function(v){return v==="known";}).length;
  var lit=Math.min(dots.length,Math.round(known*dots.length/ARCH_BASE));var maxRank={};dots.forEach(function(d){if(maxRank[d.ci]==null||d.rank>maxRank[d.ci])maxRank[d.ci]=d.rank;});
  var inc=ARCH_CLUSTERS.map(function(c,ci){return ci;}).filter(function(ci){return !(maxRank[ci]<lit);});
  if(!inc.length)return null;var best=inc[0],rem=1e9;inc.forEach(function(ci){var r=Math.ceil((maxRank[ci]+1)*ARCH_BASE/dots.length)-known;if(r<rem){rem=r;best=ci;}});
...
function _gsToken(cb,interactive){
  ...
  if(!_gsClient){
    _gsClient=google.accounts.oauth2.initTokenClient({
      ...
      callback:function(r){if(r&&r.access_token){_gsTok=r.access_token;_gsExp=Date.now()+((r.expires_in||3600)-60)*1000;SV("dks_gsync_snz",0);var f=_gsCb;_gsCb=null;_gsPaint();if(f)f();}},
      error_callback:function(){_gsCb=null;SV("dks_gsync_snz",Date.now()+24*3600*1000);_gsPaint();}
    });
  }
  _gsCb=cb;
...
function _gsSnoozed(){return (LS("dks_gsync_snz",0)||0)>Date.now();}
